...
class AccountControllerTest {
        ...
        void createAccounts_success() throws Exception {
                ...
                when(userService.getUserById(1L)).thenReturn(userDTO, userDTO);
                ...
                resp1.setIban("NL91ABNA0417164300");
                resp1.setBalance(BigDecimal.TEN);
                resp1.setAbsoluteLimit(BigDecimal.ONE);
                resp1.setTransactionLimit(BigDecimal.ONE);
                resp1.setOwner(1L);
                when(accountMapper.toResponse(any(Account.class))).thenReturn(resp1);
        ...
        void createAccounts_missingUserId_returnsError() throws Exception {
                ...
                mockMvc.perform(MockMvcRequestBuilders.post("/accounts")
                                ...
                                .andExpect(MockMvcResultMatchers.jsonPath("$.message")
                                                .value("Account owner is required"));
        }
        ...
        void getTransactionsForAccount_noTransactions_returnsError() throws Exception {
                ...
                mockMvc.perform(MockMvcRequestBuilders.get("/accounts/{iban}/transactions", iban))
                                ...
                                .andExpect(MockMvcResultMatchers.jsonPath("$.message")
                                                .value("No transactions found for this account"));
        }
        ...
        // getAccounts test cases

        @Test
        void getAccounts_success() throws Exception {
                @SuppressWarnings("unchecked")
                Page<Account> accountsPage = Mockito.mock(Page.class);
                PaginatedResponseDTO<AccountDTOResponse> paginatedResponse = new PaginatedResponseDTO<>();

                when(accountService.getAccountsByFilter(any(AccountFilter.class))).thenReturn(accountsPage);
                when(accountsPage.getSize()).thenReturn(2);
                when(accountMapper.toPaginatedResponse(accountsPage)).thenReturn(paginatedResponse);

                mockMvc.perform(MockMvcRequestBuilders.get("/accounts"))
                                .andExpect(MockMvcResultMatchers.status().isOk());

                verify(accountService).getAccountsByFilter(any(AccountFilter.class));
                verify(accountMapper).toPaginatedResponse(accountsPage);
        }

        @Test
        void getAccounts_noAccountsFound_returnsError() throws Exception {
                @SuppressWarnings("unchecked")
                Page<Account> accountsPage = Mockito.mock(Page.class);

                when(accountService.getAccountsByFilter(any(AccountFilter.class))).thenReturn(accountsPage);
                when(accountsPage.getSize()).thenReturn(-1);

                mockMvc.perform(MockMvcRequestBuilders.get("/accounts"))
                                .andExpect(MockMvcResultMatchers.status().isNotFound())
                                .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("No accounts found"));
        }

        @Test
        void getAccounts_serviceThrows_returnsError() throws Exception {
                when(accountService.getAccountsByFilter(any(AccountFilter.class)))
                                .thenThrow(new RuntimeException("Service error"));
